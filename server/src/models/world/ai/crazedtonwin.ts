...
export class CrazedTonwinAIBehavior extends DefaultAIBehavior {
  ...
  public get livingBrothers(): INPC[] {
    return this.brothers.filter((ac: INPC) => !this.game.characterHelper.isDead(ac)).filter(Boolean);
  }
  ...
  override mechanicTick(): void {
    ...
    if (this.game.effectHelper.hasEffect(npc, 'Invulnerable') && this.livingBrothers.length === 0) {
  ...
  private spawnBrother() {
    ...
    if (!spawnObj) throw new Error(`Invalid spawn obj, ${JSON.stringify(this.brotherSpawns)} is left`);
    ...
    const msgObject = { from: npc.name, message: `BROTHER ${id.split(' ')[1].toUpperCase()}! JOIN ME!`, subClass: 'chatter' };
    this.game.messageHelper.sendMessageToMap(npc.map, msgObject);

    let brother: INPC | null;

    const npcSpawner = this.game.worldManager.getMap(npc.map)?.state.getNPCSpawnerByName(`Brother Spawner ${spawnLoc + 1}`);
    if (npcSpawner) {

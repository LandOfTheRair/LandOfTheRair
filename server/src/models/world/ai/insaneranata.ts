...
export class InsaneRanataAIBehavior extends DefaultAIBehavior {
  ...
  public get livingInsanes(): INPC[] {
    return this.insanes.filter((ac: INPC) => !this.game.characterHelper.isDead(ac)).filter(Boolean);
  }
  ...
  override mechanicTick(): void {
    ...
    for (let i = 1; i < 20; i++) {
      if (npc.hp.current < npc.hp.maximum * ((i * 5) / 100) && !this.spawnTicks[i]) {
    ...
    for (let i = 1; i < 10; i++) {
      if (npc.hp.current < npc.hp.maximum * ((i * 10) / 100) && !this.agroTicks[i]) {
        this.agroTicks[i] = true;
        this.fixateOnRandom();
      }
    }

    if (this.game.effectHelper.hasEffect(npc, 'Invulnerable') && this.livingInsanes.length === 0) {
  ...
  override death(): void {
    ...
    this.game.messageHelper.sendMessageToMap(npc.map, { from: npc.name, message: 'No! Not like this! I have so many more experiments!' });

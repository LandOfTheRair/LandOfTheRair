...
import { IAIBehavior, INPC, IPlayer, IVendorBehavior, distanceFrom, MapLayer } from '../../../../interfaces';
  ...
  init(game: Game, npc: INPC, parser: Parser, behavior: IVendorBehavior) {
    ...
    parser.addCommand('hello')
      ...
      .setLogic(async ({ env }) => {
        ...
        const exits = mapData.map.tiledJSON.layers[MapLayer.Interactables].objects.filter(x => x.type === 'Teleport');
        ...
        game.teleportHelper.teleport(player as Player, { x: chosenPoint.x / 64, y: (chosenPoint.y / 64) });

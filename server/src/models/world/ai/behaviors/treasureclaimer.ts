  ...
  init(game: Game, npc: INPC, parser: Parser, behavior: ITreasureClaimer) {
    ...
    parser.addCommand('hello')
      ...
      .setLogic(async ({ env }) => {
        ...
        if (distanceFrom(player, npc) > 2) return '[The pylon requires you to be closer.]';

        if (!treasureMap) return '[The pylon is misconfigured.]';

        if (game.rngDungeonGenerator.hasClaimed(treasureMap, player.uuid)) return '[The pylon is dim towards you.]';

        const message = '[The pylon urges you to grasp at your desired treasure.]';
        ...
        const formattedChat: IDialogChatAction = {
          ...
          options: [
            { text: 'The breastplate -- I want new armor', action: 'take armor' },
    ...
    parser.addCommand('take')
      ...
      .setLogic(async ({ env, args }) => {
        ...
        if (!['weapon', 'armor', 'jewelry', 'gem'].includes(type)) return '[The pylon finds your query confusing.]';
        ...
        const item: ISimpleItem = game.itemCreator.getSimpleItem(randomItem.name);

...
import { Currency, distanceFrom, IAIBehavior, INPC, ItemSlot } from '../../../../../interfaces';
    ...
    parser.addCommand('hello')
      ...
      .setLogic(async ({ env }) => {
        ...
        if (player.items.equipment[ItemSlot.RightHand]) {
          if (player.items.equipment[ItemSlot.RightHand].name === 'Thanksgiving Turkey Feather') {

            game.characterHelper.setRightHand(player, undefined);

            if (game.diceRollerHelper.XInOneHundred(90)) {
              const results = game.lootHelper.chooseWithoutReplacement(tokenTable);
              const tokens = results[0] as unknown as number;

              game.messageHelper.sendLogMessageToPlayer(player, { message: `Koda hands you ${tokens} turkey coins!` });
              game.currencyHelper.gainCurrency(player, tokens, Currency.Thanksgiving);

            } else {
              const results = game.lootHelper.chooseWithoutReplacement(itemTable);
              const item = results[0];

              const rawItem = game.itemCreator.getSimpleItem(item);
              if (rawItem.name === 'Gold Coin') rawItem.mods.value = random(10000, 500000) as number;
        ...
        if (!checkMap.getWallAt(x, y) || checkMap.getDenseDecorAt(x, y)) {
          ...
          for (let xx = x - 2; xx <= x + 2; xx++) {
            for (let yy = y - 2; yy <= y + 2; yy++) {
              if (checkMap.getWallAt(xx, yy) || checkMap.getDenseDecorAt(x, y) || checkMap.getFluidAt(x, y)) {
                isValidSpawn = false;
              }
            }
          }

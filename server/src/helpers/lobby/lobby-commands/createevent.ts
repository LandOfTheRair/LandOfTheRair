...
export class CreateEventCommand implements ILobbyCommand {
  name = '/createevent';
  syntax = '/createevent <name="Test Event" duration=30 statBoost.skillBonusPercent=10 statBoost.xpBonusPercent=10> (duration is minutes)';
  ...
  do(message: string, game: Game) {
    ...
    if (!formattedArgs.description) formattedArgs.description = 'A GM-started festival!';

...
export class MetadataStorage {
  ...
  private static hiddenPropertiesByEntity: Record<string, Record<string, boolean>> = {};
  private static persistPropertiesByEntity: Record<string, Record<string, boolean>> = {};
  ...
  static getCollectionForEntity(entity): string {
    ...
    if (!entityName) throw new Error(`Error getting collection for entity ${entity.name}`);
  ...
  static setPropertyMetadata(entity, propName, propertyMetadata) {
    ...
    this.hiddenPropertiesByEntity[entityName] = this.hiddenPropertiesByEntity[entityName] || {};
    this.persistPropertiesByEntity[entityName] = this.persistPropertiesByEntity[entityName] || {};

    if (!isUndefined(propertyMetadata.hidden)) {
      this.hiddenPropertiesByEntity[entityName][propName] = propertyMetadata.hidden;
    }

    if (!isUndefined(propertyMetadata.persist)) {
      this.persistPropertiesByEntity[entityName][propName] = propertyMetadata.persist;
    }
  }

...
import { DamageArgs, ICharacter, INPC, IStatusEffect,
  ItemClass, MonsterClass, SharpWeaponClasses, MagicClasses } from '../../../../interfaces';
...
export class Attribute extends Effect {
  ...
  public apply(char: ICharacter, effect: IStatusEffect) {
    ...
    effInfo.tooltipColor = this.determineColor(effInfo.damageType ?? 'physical');

    if (effInfo.potency <= 0) effInfo.potency = 0;
    if (effInfo.potency === 0) effInfo.tooltip = `Immune to ${capitalize(damageType)} damage.`;

    if (damageType === 'turkey') {
      effInfo.effectIcon = 'bird-twitter';
      effInfo.tooltip = 'Immune to all damage, except from blunderbuss-type weapons.';
  ...
  public incoming(
    ...
    if (damageType === 'turkey') {
      if (attackerWeapon) {
        const { itemClass } = this.game.itemHelper.getItemProperties(attackerWeapon, ['itemClass']);
    ...
    && this.game.effectHelper.hasEffect(attacker, ResistanceShredders[monsterClass])
    && effect.effectInfo.potency < 1) return currentDamage;

    // if the attribute matches the damage type, we lower it
    if (damageClass === effInfo.damageType) return Math.floor(currentDamage * effInfo.potency);
    ...
    if (damageClass === 'physical') {
      ...
      if (attackerWeapon) {
        const { itemClass } = this.game.itemHelper.getItemProperties(attackerWeapon, ['itemClass']);
        if (damageType === 'blunt' && !SharpWeaponClasses[itemClass ?? ItemClass.Hands]) return Math.floor(currentDamage * effInfo.potency);
        if (damageType === 'sharp' && SharpWeaponClasses[itemClass ?? ItemClass.Hands])  return Math.floor(currentDamage * effInfo.potency);

      // if not, we're using hands, we only care to check blunt resist in this case
      } else {
        if (damageType === 'blunt')                                                      return Math.floor(currentDamage * effInfo.potency);
      }
    }

    if (damageType === 'magical' && MagicClasses[damageClass])                           return Math.floor(currentDamage * effInfo.potency);

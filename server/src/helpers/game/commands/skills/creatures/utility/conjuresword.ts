...
export class ConjureSword extends SpellCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    if (!args.stringArgs) {
      ...
      const options = [
        { text: 'Nevermind', action: 'noop' }
      ];

      let msg = 'Choose a weapon:';

      const weaponChoices = this.game.contentManager.getItemsMatchingName('Conjured');
      weaponChoices.forEach((weapon, i) => {
        ...
        options.push({ text: `${weaponName}`, action: `cast conjuresword ${weaponName}` });
      });

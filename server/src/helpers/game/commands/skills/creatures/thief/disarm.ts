import { ICharacter, IMacroCommandArgs, IPlayer, Skill } from '../../../../../../interfaces';
...
export class Disarm extends SkillCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    const trapEffect = this.game.itemHelper.getItemProperty(trapItem, 'trapEffect');
    if (trapItem.mods.trapSetBy === player.uuid) return this.sendMessage(player, 'You cannot disarm your own traps!');
    ...
    this.sendMessage(player, `You disarmed the ${trapEffect?.name ?? 'unknown'} trap.`);

import { IMacroCommandArgs, IPlayer, ItemClass, ObjectType, SwimLevel } from '../../../../interfaces';
...
export class Fill extends MacroCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    if (!['right', 'left'].includes(args.stringArgs)) return this.sendMessage(player, 'You do not have anything to fill there!');
    ...
    if ((ounces ?? 0) > 0) return this.sendMessage(player, 'That bottle is not empty!');

...
import { GameServerResponse, IDialogChatAction, IMacroCommandArgs,
  IPlayer, ISimpleItem, ItemClass, ItemSlot, SoundEffect, Tradeskill } from '../../../../interfaces';
...
export class Shatter extends MacroCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    if (item && args.stringArgs) {
      return this.sendMessage(player, 'You need to empty your right hand to mass shatter!');
    }
    ...
    if (!item) {
      ...
      if (!args.stringArgs) {
        ...
        const options: string[] = uniq(player.items.sack.items
          .filter(x => this.game.itemHelper.getItemProperty(x, 'itemClass') === ItemClass.Gem)
          .map(x => this.game.itemHelper.getItemProperty(x, 'itemClass')))
          .sort();

        if (options.length === 0) return this.sendMessage(player, 'You do not have any shatterable items in your sack!');

        const message = 'What would you like to shatter all of (from your sack)?';
      ...
      if (args.stringArgs) {
        ...
        if (items.length === 0) return this.sendMessage(player, 'You do not have any matching shatterable items in your sack!');
        ...
        items.forEach(cItem => gemDust.mods.ounces = (gemDust.mods.ounces ?? 0) + getOzFromItem(cItem));

        this.game.characterHelper.setEquipmentSlot(player, ItemSlot.RightHand, gemDust);

        const skill = this.game.calculatorHelper.calcTradeskillLevelForCharacter(player, Tradeskill.Gemcrafting);
        if (skill < 10) {
          this.game.playerHelper.gainTradeskill(player, Tradeskill.Gemcrafting, gemDust.mods.ounces);
        }
    ...
    if (item && !args.stringArgs) {
      ...
      if (itemClass !== ItemClass.Gem) return this.sendMessage(player, 'That is not shatterable!');
      if (!this.game.itemHelper.isOwnedBy(player, item)) return this.sendMessage(player, 'That item is not yours to shatter!');
      ...
      this.game.characterHelper.setEquipmentSlot(player, ItemSlot.RightHand, gemDust);
      this.game.playerHelper.gainTradeskill(player, Tradeskill.Gemcrafting, oz);

      this.sendMessage(player, `You shatter the item in your right hand and get ${oz}oz dust!`, SoundEffect.CombatBlockArmor);
    }
  }
}

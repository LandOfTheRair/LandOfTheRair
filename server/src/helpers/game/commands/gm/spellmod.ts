...
export class GMSpellMod extends MacroCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    if (!name) return this.sendMessage(player, 'You must specify a spell name or "list".');
    ...
    if (name === 'list') {
      ...
      Object.keys(allOverrides).forEach(spell => {
        this.sendMessage(player, `* ${spell}: ${allOverrides[spell]} (${allOverrides[spell] * 100}%)`);
      });
      return;
    }
    ...
    this.sendMessage(player, `${name} spell potency set to ${setPotency === 0 ? 'default' : (setPotency * 100) + '%'}.`);
  }
}

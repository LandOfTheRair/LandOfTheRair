import { Hostility, ICharacter, INPC, ItemClass, SpellCastArgs, Stat } from '../../../../interfaces';
...
export class Resurrect extends Spell {
  ...
  private spawnZombie(caster: ICharacter, level: number): void {
    ...
    const npcCreateCallback = (npc: INPC) => {
      ...
      const skillBoost = this.game.calculatorHelper.calculateSkillXPRequiredForLevel(Math.floor(level / 2));
      Object.keys(npc.skills).forEach(skillName => {
        npc.skills[skillName] += skillBoost;
      });

      npc.stats[Stat.HP] = (npc.stats[Stat.HP] ?? 20000) * 1 + this.game.traitHelper.traitLevelValue(caster, 'FamiliarFortitude');
      npc.stats[Stat.STR] = (npc.stats[Stat.STR] ?? 5) + this.game.traitHelper.traitLevelValue(caster, 'FamiliarStrength');
      npc.stats[Stat.INT] = (npc.stats[Stat.INT] ?? 5) + this.game.traitHelper.traitLevelValue(caster, 'FamiliarStrength');
      npc.stats[Stat.WIS] = (npc.stats[Stat.WIS] ?? 5) + this.game.traitHelper.traitLevelValue(caster, 'FamiliarStrength');

import { foodTextFor, GameServerResponse, ICharacter, IPlayer, ItemClass, ItemSlot, SpellCastArgs } from '../../../../interfaces';
...
export class Foodsense extends Spell {
  ...
  override cast(caster: ICharacter | null, target: ICharacter | null, spellCastArgs: SpellCastArgs): void {
    ...
    const { itemClass, useEffect } = this.game.itemHelper.getItemProperties(rightHand, ['itemClass', 'useEffect']);

    if (itemClass !== ItemClass.Food) {
      this.sendMessage(caster, { message: 'You do not have a food item in your right hand!' });
      return;
    }

...
export class MarketRoute {
  ...
  static setup(fastify: any, { database }: { database: Database }) {
    ...
    fastify.all('/market/listings/all', async (req, res) => {
      ...
      const search = await listings.find(query).sort(sort).skip(page * 50).limit(50).toArray();
      res.send(search);
    });

    fastify.all('/market/listings/mine', async (req, res) => {
      const myListings = await listings.find({ 'listingInfo.seller': req.query.username }).sort({ 'listingInfo.listedAt': -1 }).toArray();
      res.send(myListings);
    });

    fastify.all('/market/pickups/mine', async (req, res) => {
      const myPickups = await pickups.find({ username: req.query.username }).toArray();
      res.send(myPickups);
    });
  }

...
export class VerifyRequestAction extends ServerAction {
  ...
  override async act(game: Game, callbacks, data) {
    ...
    if (account.verificationCode) return { wasSuccess: false, message: 'You already have a verification code active. Please wait.' };
    ...
    return { wasSuccess: true, message: `A verification code has been sent to ${account.email}!` };
  }
}

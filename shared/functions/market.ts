import { Currency, IItem, IPlayer, ISimpleItem, ItemClass } from '../interfaces';
...
export function calculateListingFee(item: IItem, sellPrice: number): number {
  return Math.max(1, Math.floor((sellPrice + (item.sellValue || item.value)) * listingFeePercent()));
}
...
export function itemListError(player: IPlayer, item: ISimpleItem, realItem: IItem, price: number): string {
  ...
  if (item.mods.encrustItem) return 'That item is encrusted and cannot be sold.';

  if (item.mods.destroyOnDrop || realItem.destroyOnDrop) return 'That item is too fragile to be sold.';

...
import { Direction } from '@lotr/interfaces';
 ...
 */
export function positionSubtract(
  position1: IPosition,
  position2: IPosition,
): IPosition {
 ...
 * Converts a world position into a new tile position
 */
export function positionWorldXYToTile({
  worldX,
  worldY,
}: {
  worldX: number;
 ...
 */
export function positionSurrounding(): readonly (IPosition & {
  direction: Direction;
})[] {
  return surrounding;
}
...
export function positionInRange(
  center: IPosition,
  range: number,
  method: (position: IPosition) => void,
) {

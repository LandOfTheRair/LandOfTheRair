    ...
    parser
      ...
      .setLogic(async ({ env }) => {
        ...
        const brainIndexes = player.items.sack.items.filter(
          (x: ISimpleItem) => x.name === 'Halloween Zombie Brain',
        );
        const pileIndexes = player.items.sack.items.filter(
          (x: ISimpleItem) => x.name === 'Halloween Candy Pile',
        );
        const candyIndexes = player.items.sack.items.filter((x: ISimpleItem) =>
          x.name.includes('Halloween Candy -'),
        );

        game.inventoryHelper.removeItemsFromSackByUUID(player, [
          ...brainIndexes.map((x: ISimpleItem) => x.uuid),
          ...pileIndexes.map((x: ISimpleItem) => x.uuid),
          ...candyIndexes.map((x: ISimpleItem) => x.uuid),
        ]);

  ...
  init(
    ...
    parser
      ...
      .setLogic(async ({ env }) => {
        ...
        Object.keys(allAvailable).forEach((key) => {
          if ((allAvailable[key as keyof typeof allAvailable] ?? 0) <= 0) {
            return;
          }

          options.push({
            text: `${key} (${allAvailable[key as keyof typeof allAvailable]})`,
            action: `take ${key}`,
          });
        });
    ...
    parser
      ...
      .setLogic(async ({ env, args }) => {
        ...
        if ((allAvailable[cosmetic as keyof typeof allAvailable] ?? 0) <= 0) {

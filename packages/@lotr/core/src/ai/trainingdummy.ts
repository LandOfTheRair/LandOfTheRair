...
export class TrainingDummyAI extends DefaultAIBehavior {
  ...
  override mechanicTick(): void {
    ...
    Object.keys(this.currentlyTicking).forEach((username) => {
      ...
      if (this.currentlyTicking[username]! <= 0) {
        const totalDamageForPeriod = this.runningDamageTotals[username] ?? 0;
  ...
  override damageTaken({
    ...
    this.currentlyTicking[playerUsername] =
      this.currentlyTicking[playerUsername] || 30;

    this.runningDamageTotals[playerUsername] =
      this.runningDamageTotals[playerUsername] || 0;
    this.runningDamageTotals[playerUsername] += damage;

    this.tenSecDamageTotals[playerUsername] =
      this.tenSecDamageTotals[playerUsername] || 0;

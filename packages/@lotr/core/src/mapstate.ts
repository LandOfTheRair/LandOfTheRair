...
export class MapState implements IMapState {
  ...
  public get allSpawners(): ISpawner[] {
  ...
  private createDefaultSpawner() {
    ...
    npcDefs.forEach((def: any, i: number) => {
  ...
  private createOtherSpawners() {
    ...
    this.map.allSpawners.forEach((spawner: any) => {
  ...
  public removeSpawner(spawner: Spawner) {
    ...
    if (spawner.id) {
      delete this.spawnersById[spawner.id];
    }

    if (spawner.spawnerName) {
      delete this.spawnersByName[spawner.spawnerName];
    }
  }
  ...
  public isDoorOpen(id: number): boolean {
  ...
  private getPlayersFromQuadtrees(
    ...
    return this.players
      ...
      .map(({ uuid }: any) => this.playersByUUID[uuid])
      .filter(Boolean) as IPlayer[];
  }
  ...
  private getNPCsFromQuadtrees(
    ...
    return this.npcs
      ...
      .map(({ uuid }: any) => this.npcsByUUID[uuid])
      .filter(Boolean) as INPC[];
  }
  ...
  public getPlayersToUpdate(x: number, y: number): IPlayer[] {
  ...
  public moveNPCOrPlayer(
    character: ICharacter,
    { oldX, oldY }: { oldX: number; oldY: number },
  ): void {
    if (isPlayer(character)) {
      this.movePlayer(character as IPlayer, { oldX, oldY });
    } else {
  ...
  private triggerPlayerUpdateForPlayer(player: IPlayer) {
    ...
    const nearbyPlayers = this.players
      ...
      .filter(({ uuid }: any) => uuid !== player.uuid)
      .filter((p: any) =>
      ...
      .map(({ uuid }: any) => pick(this.playersByUUID[uuid], PLAYER_KEYS))
      .filter(Boolean);
  ...
  private triggerNPCUpdateForPlayer(player: IPlayer) {
    ...
    const nearbyNPCs = this.npcs
      ...
      .map(({ uuid }: any) => pick(this.npcsByUUID[uuid], NPC_KEYS))
      .filter(Boolean);
  ...
  private moveNPC(npc: INPC, { oldX, oldY }: { oldX: number; oldY: number }) {
  ...
  public getNPCSpawnersByName(name: string): ISpawner[] {

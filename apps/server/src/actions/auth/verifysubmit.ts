...
export class VerifySubmitAction extends ServerAction {
  ...
  override async act(game: Game, callbacks, data) {
    ...
    if (!account.verificationCode) {
      return {
        wasSuccess: false,
        message: 'You must first request a verification code.',
      };
    }

    if (Date.now() > account.verificationExpiration) {
      return { wasSuccess: false, message: 'Verification code expired.' };
    }
    ...
    if (account.verificationAttempts > 5) {
      ...
      return {
        wasSuccess: false,
        message: 'Too many attempts with wrong code.',
      };
    }

    if (account.verificationCode !== data.verificationCode) {
      return { wasSuccess: false, message: 'Invalid verification code.' };
    }

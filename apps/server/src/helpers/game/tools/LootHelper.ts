...
export class LootHelper extends BaseService {
  ...
  public chooseWithoutReplacement(
    choices: Rollable[] | string[],
    number = 1,
    bonus = 0,
  ) {
  ...
  public getNPCLoot(npc: INPC, bonus = 0): ISimpleItem[] {
    ...
    if (!isNaturalResource && regionDroptables.drops?.length > 0) {
      rolledResults.push(
        ...this.tryEachItem(this.filterDropTable(regionDroptables.drops)),
      );
    }
    ...
    if (!isNaturalResource && mapDroptables.drops?.length > 0) {
      rolledResults.push(
        ...this.tryEachItem(this.filterDropTable(mapDroptables.drops)),
      );
    }
    ...
    if ((npc.drops?.length ?? 0) > 0) {
      rolledResults.push(
        ...this.tryEachItem(this.filterDropTable(npc.drops ?? [])),
      );
    ...
    if ((npc.copyDrops?.length ?? 0) > 0) {
      ...
      if (drops.length > 0) {
        rolledResults.push(
          ...this.tryEachItem(this.filterDropTable(drops as Rollable[])),
        );
      }
    }
    ...
    if (npc.dropPool) {
      ...
      while (numChoices > 0 && items.length > 0) {
        ...
        const results = replace
          ? this.chooseWithReplacement(filtered)
          : this.chooseWithoutReplacement(filtered);
    ...
    const rolledItems: ISimpleItem[] = rolledResults.map((x) =>
      this.game.itemCreator.getSimpleItem(x),
    );

...
export class Applied extends Effect {
  public override create(char: ICharacter, effect: IStatusEffect) {
    this.sendMessage(char, {
      message: `Your weapons are now coated with ${effect.effectInfo.applyEffect?.name}.`,
    });
...
return;
    ...
    this.game.effectHelper.addEffect(target, char, aEffect.name, {
      effect: {
        duration: aEffect.duration,
        extra: { potency: aEffect.potency },
      },
    });
  }
}

...
export class ExcogitativeAura extends Effect {
  public override create(char: ICharacter, effect: IStatusEffect) {
    ...
    if (center) {
      this.game.messageHelper.sendLogMessageToRadius(center, 5, {
        message: 'You see powerful light envelop the area near you!',
      });
    }
  }
  ...
  public override tick(char: ICharacter, effect: IStatusEffect) {
    ...
    if (center) {
      this.game.targettingHelper
        ...
        .forEach((target) => {
          const gotEffect = getEffect(target, 'Excogitation');
          if (!gotEffect) {
            this.game.effectHelper.addEffect(target, char, 'Excogitation', {
              effect: {
                extra: {
      ...
      if (distanceFrom(char, center) > 5) {
        ...
        this.sendMessage(char, { message: 'Your powerful light dissipates!' });
        return;
      }
    }
  }

...
return this.sendMessage(player, 'You are already in a party!');
    ...
    const leaderPlayer = this.game.playerManager.getPlayerByUsername(
      party.members[0],
    );
    if (!leaderPlayer) {
return this.sendMessage(player, 'Somehow, that person is not in game!');
}

    if (!this.game.targettingHelper.isVisibleTo(player, leaderPlayer, true)) {
return this.sendMessage(player, 'You do not see the leader!');
}

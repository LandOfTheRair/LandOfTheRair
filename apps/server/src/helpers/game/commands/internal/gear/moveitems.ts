...
const allAliases = origins
  .map((o) => validDestinations[o].map((sub) => `${o}t${sub}`))
  .flat();
...
export class MoveItems extends MacroCommand {
  ...
  override execute(player: IPlayer, args: IMacroCommandArgs) {
    ...
    if (!this[`handle${o}`]) {
      return this.sendMessage(player, 'Invalid item move origin.');
    }
  ...
  private doPostChecks(
    ...
    if (updatePlayerSlots[srcSlot] || updatePlayerSlots[destSlot]) {
      state.triggerPlayerUpdateInRadius(player.x, player.y);
    }
    if (updateGroundSlots[srcSlot] || updateGroundSlots[destSlot]) {
      state.triggerGroundUpdateInRadius(player.x, player.y);
    }
  ...
  private doPrelimChecks(
    ...
    if (dest === 'G' && succorInfo) {
      ...
      if (src === 'S') {
        this.game.inventoryHelper.removeItemsFromSackByUUID(player, [
          srcItem.uuid,
        ]);
      } else if (src === 'B') {
        this.game.inventoryHelper.removeItemsFromBeltByUUID(player, [
          srcItem.uuid,
        ]);
      } else if (src === 'D') {
        this.game.inventoryHelper.removeItemsFromPouchByUUID(player, [
          srcItem.uuid,
        ]);
      }
    ...
    if (
      dest === 'S' &&
      !this.game.inventoryHelper.canAddItemToSack(player, srcItem)
    ) {
    ...
    if (
      dest === 'B' &&
      !this.game.inventoryHelper.canAddItemToBelt(player, srcItem)
    ) {
    ...
    if (dest === 'M') {
      ...
      const npc: INPC =
        this.game.targettingHelper.getFirstPossibleTargetInViewRange(
          player,
          destSlot,
        ) as INPC;
      if (!npc) {
      ...
      const vendorBehavior: VendorBehavior = (npc.behaviors || []).find(
        (b) => (b as VendorBehavior).vendorItems,
      ) as VendorBehavior;
      if (!vendorBehavior) {
    ...
    if (src === 'O') {
      ...
      const npc = this.game.targettingHelper.getFirstPossibleTargetInViewRange(
        player,
        npcUUID,
      );
      if (!npc) {
    ...
    if (dest === 'W' || src === 'W') {
      ...
      if (src === 'W') {
        if (!this.game.lockerHelper.hasLockerFromString(player, srcSlot)) {
          return false;
        }
      }
      ...
      if (dest === 'W') {
        ...
        const lockerRef = this.game.lockerHelper.getLockerFromString(
          player,
          destSlot,
        );
        const canAdd = this.game.inventoryHelper.canAddItemToLocker(
          player,
          srcItem,
          lockerRef,
        );
  ...
  private getEquipmentSlot(
    ...
    if (slot === ItemSlot.Robe) {
      if (
        player.items.equipment[ItemSlot.Armor] &&
  ...
  handleC(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.C.includes(dest)) {
    ...
    switch (dest) {
      ...
      default: {
        ...
        return this.sendMessage(
          player,
          'Something went wrong, please contact a GM.',
        );
      }
    }
  }
  ...
  handleL(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.L.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    const srcItem = player.items.equipment[ItemSlot.LeftHand];

    if (!this.doPrelimChecks(player, srcItem, 'L', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      ...
      case 'S': {
        ...
        if (!srcItem) {
          return this.sendMessage(
            player,
            "You aren't holding anything in that hand!",
          );
        }
        if (!this.game.inventoryHelper.canAddItemToSack(player, srcItem)) {
          return this.sendMessage(player, 'Your sack is full.');
        }
      ...
      case 'D': {
        ...
        if (!this.game.inventoryHelper.canAddItemToPouch(player, srcItem)) {
          return this.sendMessage(player, 'Your pouch is full.');
        }
      ...
      case 'B': {
        ...
        if (!this.game.inventoryHelper.canAddItemToBelt(player, srcItem)) {
          return this.sendMessage(player, 'Your belt is full.');
        }
      ...
      case 'E': {
        ...
        if (!equipSlot) {
          return this.sendMessage(player, "That item doesn't fit there.");
        }

        if (player.items.equipment[equipSlot]) {
          return this.sendMessage(
            player,
      ...
      case 'M': {
        ...
        if (!this.game.inventoryHelper.canSellItem(player, srcItem)) {
          return this.sendMessage(player, "The merchant won't accept that.");
        }
      ...
      case 'W': {
        ...
        this.game.inventoryHelper.addItemToLocker(
          player,
          srcItem,
          this.game.lockerHelper.getLockerFromString(player, destSlot),
        );
        this.game.characterHelper.setLeftHand(player, undefined);
        break;
      }
      ...
      case 'K': {
        ...
        if (!materialRef) {
          return this.sendMessage(player, 'That is not a material!');
        }
        ...
        if (materialSpaceLeft <= 0) {
          return this.sendMessage(
            player,
            'You have no more space for that material!',
          );
        }
  ...
  handleR(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.R.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    const srcItem = player.items.equipment[ItemSlot.RightHand];

    if (!this.doPrelimChecks(player, srcItem, 'R', origSlot, dest, destSlot)) {
      return;
    }
      ...
      case 'K': {
        ...
        if (withdrawInOunces) {
          ...
          if (materialSpaceLeft <= 0) {
            return this.sendMessage(
              player,
              'You have no more space for that item!',
            );
          }
  ...
  handleE(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.E.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!this.doPrelimChecks(player, srcItem, 'E', origSlot, dest, destSlot)) {
      return;
    }
  ...
  handleB(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.B.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!srcItem) {
      return this.sendMessage(player, "You don't have an item there!");
    }

    if (!this.doPrelimChecks(player, srcItem, 'B', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      case 'R': {
        ...
        if (rightHand && !leftHand) {
          const did = this.game.inventoryHelper.removeItemFromBelt(
            player,
            +origSlot,
          );
          if (!did) {
            return this.sendMessage(player, 'Could not take item from belt.');
          }
  ...
  handleS(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.S.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!this.doPrelimChecks(player, srcItem, 'S', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      case 'R': {
        ...
        if (rightHand && !leftHand) {
          const did = this.game.inventoryHelper.removeItemFromSack(
            player,
            +origSlot,
          );
          if (!did) {
            return this.sendMessage(player, 'Could not take item from sack.');
          }
  ...
  handleG(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.G.includes(dest) || !origSlot) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (items.length === 0) {
      return this.sendMessage(player, 'No items to grab.');
    }

    if (
      !this.doPrelimChecks(player, items[0].item, 'G', origSlot, dest, destSlot)
    ) {
      return;
    }
    ...
    switch (dest) {
      case 'R': {
        ...
        const functionalItem = this.game.itemCreator.rerollItem(
          items[0].item,
          false,
        );
      ...
      case 'S': {
        ...
        items.forEach((item) => {
          ...
          if (
            checkItemClass !== ItemClass.Coin &&
            addItems.length >= spaceLeft
          ) {
            return;
          }
          ...
          for (let i = 0; i < item.count; i++) {
            ...
            uuidRemoveCounts[item.item.uuid] =
              uuidRemoveCounts[item.item.uuid] || 0;
            uuidRemoveCounts[item.item.uuid]++;
        ...
        addItems.forEach((item) => {
          if (!this.game.inventoryHelper.canAddItemToSack(player, item)) {
            return this.sendMessage(player, 'Your sack is full.');
          }
      ...
      case 'D': {
        ...
        addItems.forEach((item) => {
          if (!this.game.inventoryHelper.canAddItemToPouch(player, item)) {
            return this.sendMessage(player, 'Your pouch is full.');
          }
      ...
      case 'B': {
        ...
        addItems.forEach((item) => {
          if (!this.game.inventoryHelper.canAddItemToBelt(player, item)) {
            return this.sendMessage(player, 'Your belt is full.');
          }
      ...
      case 'W': {
        ...
        const destLocker = this.game.lockerHelper.getLockerFromString(
          player,
          destSlot,
        );
        const spaceLeft = this.game.inventoryHelper.lockerSpaceLeft(
          player,
          destLocker,
        );
        ...
        addItems.forEach((item) => {
          if (
            !this.game.inventoryHelper.canAddItemToLocker(
              player,
              item,
              destLocker,
            )
          ) {
            return this.sendMessage(player, 'Your locker is full.');
          }
      ...
      case 'K': {
        ...
        items.forEach((item) => {
          const materialRef = this.game.lockerHelper.getMaterialRef(
            item.item.name,
          );
          if (!materialRef) {
            return this.sendMessage(player, 'That is not a material!');
          ...
          if (withdrawInOunces) {
            ...
            this.game.inventoryHelper.addMaterial(
              player,
              materialRef,
              totalPossible,
            );
  ...
  handleM(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.M.includes(dest) || !origSlot) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!this.doPrelimChecks(player, item, 'M', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      ...
      case 'B': {
        // MtB
        const beltSpaceLeft = Math.min(
          this.game.inventoryHelper.beltSpaceLeft(player),
          +destSlot,
        );

        const maxItemsBuyable = isDaily ? 1 : beltSpaceLeft;
      ...
      case 'S': {
        // MtS
        const sackSpaceLeft = Math.min(
          this.game.inventoryHelper.sackSpaceLeft(player),
          +destSlot,
        );
        const maxItemsBuyable = isDaily ? 1 : sackSpaceLeft;
        for (let i = 0; i < maxItemsBuyable; i++) {
          const createdItem = this.game.itemCreator.getSimpleItem(item.name);
          createdItem.mods.sellValue = cost;
      ...
      case 'D': {
        // MtD
        const pouchSpaceLeft = Math.min(
          this.game.inventoryHelper.pouchSpaceLeft(player),
          +destSlot,
        );
        const maxItemsBuyable = isDaily ? 1 : pouchSpaceLeft;
        for (let i = 0; i < maxItemsBuyable; i++) {
          const createdItem = this.game.itemCreator.getSimpleItem(item.name);
          createdItem.mods.sellValue = cost;
  ...
  handleO(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.O.includes(dest) || !origSlot) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!this.doPrelimChecks(player, srcItem, 'O', origSlot, dest, destSlot)) {
      return;
    }
  ...
  handleW(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.W.includes(dest) || !origSlot) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }

    const origLocker = this.game.lockerHelper.getLockerFromString(
      player,
      origSlot,
    );
    const [w, locker, origLockerSlot] = origSlot.split(':');
    const srcItem = origLocker.items[+origLockerSlot];
    if (!this.doPrelimChecks(player, srcItem, 'W', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      case 'R': {
        // WtR
        const didTake = this.game.inventoryHelper.removeItemFromLocker(
          player,
          +origLockerSlot,
          origLocker,
        );
        if (!didTake) {
          return this.sendMessage(player, 'Could not take item from locker.');
        }
  ...
  handleK(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.K.includes(dest) || !origSlot) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }

    const [k, source] = origSlot.split(':');
    const { items, withdrawInOunces } =
    ...
    if (totalToTake === 0) {
      return this.sendMessage(player, 'You cannot take that many items!');
    }
  ...
  handleD(player: IPlayer, dest: string, origSlot: string, destSlot: string) {
    if (!validDestinations.D.includes(dest)) {
      return this.sendMessage(player, 'Invalid item move destination.');
    }
    ...
    if (!this.doPrelimChecks(player, srcItem, 'D', origSlot, dest, destSlot)) {
      return;
    }
    ...
    switch (dest) {
      case 'R': {
        ...
        if (rightHand && !leftHand) {
          const did = this.game.inventoryHelper.removeItemFromPouch(
            player,
            +origSlot,
          );
          if (!did) {

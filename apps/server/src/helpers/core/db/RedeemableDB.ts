...
export class RedeemableDB extends BaseService {
  public async init() {
    const coll = this.game.db.getCollection(Redeemable);
  ...
  public async getRedeemable(code: string): Promise<Redeemable | null> {
    return this.game.db.findSingle<Redeemable>(Redeemable, { code });
  }
  ...
  public async claimRedeemable(
    ...
    const redeemable = await this.game.db.findSingle<Redeemable>(Redeemable, {
      code,
    });
    if (!redeemable) {
    ...
    await this.game.db.save(redeemable);

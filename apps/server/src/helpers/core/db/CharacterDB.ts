...
export class CharacterDB extends BaseService {
  public async init() {
    const coll = this.game.db.getCollection(Player);
    coll.createIndex({ name: 1 });

    const stats = this.game.db.getCollection(PlayerStatistics);
  ...
  public async createCharacter(
    ...
    const characterDetails = this.game.characterRoller.rollCharacter({
      allegiance,
      baseclass,
      weapons,
    });
  ...
  public async loadPlayers(account: Account): Promise<Player[]> {
    const players = await this.game.db.findMany<Player>(Player, {
  ...
  public async reloadPlayerAccountInfo(
    ...
    const results = await Promise.all([
      this.game.db.findSingle<AccountBank>(AccountBank, {
        _account: account._id,
      }),
      this.game.db.findSingle<AccountLockers>(AccountLockers, {
        _account: account._id,
      }),
      this.game.db.findSingle<AccountAchievements>(AccountAchievements, {
        _account: account._id,
      }),
    ]);
  ...
  public async loadPlayerDailyInfo(
    ...
    const daily = await this.game.db.findSingle<AccountDaily>(AccountDaily, {
      _account: account._id,
    });
    if (!daily) {
  ...
  public async populatePlayer(player: Player, account: Account): Promise<void> {
    const results = await Promise.all([
      this.game.db.findSingle<PlayerItems>(PlayerItems, { _id: player._items }),
      this.game.db.findSingle<PlayerTraits>(PlayerTraits, {
        _id: player._traits,
      }),
      this.game.db.findSingle<PlayerQuests>(PlayerQuests, {
        _id: player._quests,
      }),
      this.game.db.findSingle<PlayerStatistics>(PlayerStatistics, {
        _id: player._statistics,
      }),
      this.game.db.findSingle<PlayerLockers>(PlayerLockers, {
        _id: player._lockers,
      }),
    ]);
  ...
  public async deletePlayer(player: Player): Promise<void> {
    await Promise.all([
      this.game.db.delete(player),
      this.game.db.delete(player.items as PlayerItems),
      this.game.db.delete(player.traits as PlayerTraits),
      this.game.db.delete(player.quests as PlayerQuests),
      this.game.db.delete(player.statistics as PlayerStatistics),
      this.game.db.delete(player.lockers as PlayerLockers),
    ]);
  }
  ...
  public async saveAllPlayers(players: Player[]): Promise<any> {
    ...
    const playerColl = this.game.db.getCollection(Player);
    const itemsColl = this.game.db.getCollection(PlayerItems);
    const traitsColl = this.game.db.getCollection(PlayerTraits);
    const questsColl = this.game.db.getCollection(PlayerQuests);
    const statsColl = this.game.db.getCollection(PlayerStatistics);
    const lockersColl = this.game.db.getCollection(PlayerLockers);
    const bankColl = this.game.db.getCollection(AccountBank);
    const acctLockerColl = this.game.db.getCollection(AccountLockers);
    const acctDailyColl = this.game.db.getCollection(AccountDaily);
    const achievementsColl = this.game.db.getCollection(AccountAchievements);
    ...
    players.forEach((player) => {
      playerOp
        .find({ _id: player._id })
        .upsert()
        .replaceOne(this.game.db.getPersistObject(player));

      itemOp
        .find({ _id: (player.items as PlayerItems)._id })
        .upsert()
        .replaceOne(this.game.db.getPersistObject(player.items as PlayerItems));

      traitOp
        .find({ _id: (player.traits as PlayerTraits)._id })
        .upsert()
        .replaceOne(
          this.game.db.getPersistObject(player.traits as PlayerTraits),
        );
      ...
      questOp
        ...
        .upsert()
        .replaceOne(
          this.game.db.getPersistObject(player.quests as PlayerQuests),
        );
      ...
      statsOp
        ...
        .upsert()
        .replaceOne(
          this.game.db.getPersistObject(player.statistics as PlayerStatistics),
        );
      ...
      lockersOp
        ...
        .upsert()
        .replaceOne(
          this.game.db.getPersistObject(player.lockers as PlayerLockers),
        );

      bankOp
        .find({ _account: player._account })
        .upsert()
        .replaceOne(this.game.db.getPersistObject(player.bank as AccountBank));

      acctLockerOp
        .find({ _account: player._account })
        .upsert()
        .replaceOne(
          this.game.db.getPersistObject(
            player.accountLockers as AccountLockers,
          ),
        );
      ...
      achievementsOp
        ...
        .replaceOne(
          this.game.db.getPersistObject(
            player.achievements as AccountAchievements,
          ),
        );
    });
  ...
  public async savePlayer(player: Player): Promise<void> {
    ...
    const saves = [
      ...
      this.game.db.save(player.items as PlayerItems),
      this.game.db.save(player.traits as PlayerTraits),
      this.game.db.save(player.quests as PlayerQuests),
      this.game.db.save(player.statistics as PlayerStatistics),
      this.game.db.save(player.lockers as PlayerLockers),
      this.game.db.getCollection(AccountDaily).updateOne(
    ...
    if (player.bank) saves.push(this.game.db.save(player.bank as AccountBank));

    if (player.accountLockers) {
      saves.push(this.game.db.save(player.accountLockers as AccountLockers));
    }

    if (player.achievements) {
      saves.push(this.game.db.save(player.achievements as AccountAchievements));
    }

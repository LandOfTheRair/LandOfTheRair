...
import { IAccount, IClassTraitTree, IPlayer, isSubscribed, ITrait, ITraitTreeTrait } from '../../../../interfaces';
...
export class TraitsComponent implements OnInit, OnDestroy {
  ...
  private setAccount(account: IAccount) {
    ...
    this.buildSlots = Array(numSlots).fill(0).map((_, i) => i);
  }
  ...
  public loadBuild(tree: number) {
    ...
    this.modalService.confirm('Load Build', `Are you sure you want to load this build from slot ${tree + 1}?`)
      .subscribe(res => {
        if (!res) return;
  ...
  public saveBuild(tree: number) {
    ...
    this.modalService.confirm('Save Build', `Are you sure you want to save this build in slot ${tree + 1}?`)
      .subscribe(res => {
        if (!res) return;
